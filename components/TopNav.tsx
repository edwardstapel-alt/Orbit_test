...
export const TopNav: React.FC<TopNavProps> = ({ title, subtitle, onProfileClick, onMenuClick, onBack, showBack, onSearchClick, showSearch = true }) => {
  ...
  return (
    <header 
      className="sticky top-0 z-30 bg-background/95 backdrop-blur-md px-6 md:px-12 lg:px-8 xl:px-12 pt-4 lg:pt-6 pb-2 flex items-center justify-between transition-all duration-300 border-b border-transparent"
      style={{ paddingTop: 'calc(env(safe-area-inset-top, 30px) + 1rem)' }}
    >
      {showBack && onBack ? (
        <button 
